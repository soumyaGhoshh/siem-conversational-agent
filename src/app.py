...
with st.sidebar:
    ...
    role = st.radio("Role", ["analyst", "admin"], index=0)
    if st.button("Login"):
    ...
    allowed = os.getenv("ALLOWED_INDEXES", "wazuh-alerts-*")
    ...
    qb_field = st.text_input("Field")
    qb_op = st.selectbox("Operator", ["term", "match", "wildcard"]) 
    ...
    if st.button("Run Builder"):
        ...
        try:
            ...
            ok, errs = validate_dsl(dsl, fields, types_map=types_map, max_days=max_days)
            if not ok:
                st.error("; ".join(errs))
            else:
    ...
    with st.chat_message("assistant"):
        with st.spinner("Analyzing..."):
            ...
            if "error" in response:
                ...
                tabs = st.tabs(["Overview", "Results", "Query DSL", "Audit", "Saved Searches"])
                with tabs[0]:
                    ...
                    if agg_field:
                        ...
                        try:
                            ...
                            st.table({"value": [b.get('key') for b in buckets], "count": [b.get('doc_count') for b in buckets]})
                        except Exception:
                            pass
                    # Timeline
                    histo_dsl = {
                    ...
                    try:
                        ...
                        st.line_chart({"count": [b.get('doc_count') for b in hb]})
                    except Exception:
                        pass
